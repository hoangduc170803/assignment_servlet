        ...
        <c:choose>
            ...
            <c:otherwise>
                ...
                <c:if test="${not empty listC}">
                    <h2>Categories:</h2>
                    <form id="f" action="viewBook" method="get">
                        <div class="form-group row" style="margin-bottom: 20px">
                            <label class="col-sm-2 col-form-label"for="categoryId">Categories:</label>
                            <select class="col-sm-10" id="categoryId" name="id" onchange="document.getElementById('f').submit()" required>
                                <option value="-1" ${param.id == '-1' ? 'selected' : ''}>All categories</option>
                                <c:forEach items="${listC.keySet()}" var="catId">
                                    <option value="${catId}" ${catId == param.id ? 'selected' : ''}>${listC[catId].name}</option>
                                </c:forEach>
                            </select>
                        </div>
                ...
                <c:if test="${empty listB}">
                    <p style="color: red;">No books found for the selected category.</p>
                </c:if>
